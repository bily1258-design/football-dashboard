// script.js — 足彩价值投注看板 v9 (tabs: 北单 + 香港马会)
var allData = null;
var allMatches = [];
var activeTab = 'beidan';
...
// HKJC单独展示：马会赔率(初/即/分歧) 一列搞定
function renderHkjcOddsFull(h){
  if(!h || !h.current) return '<span class="odds-val odds-w">--</span>';
  var o=h.open, cur=h.current, d=h.div_pct;
  // 初盘
  var openHtml = '<span class="oc-open">'+o[0].toFixed(2)+'</span><span class="oc-sep">/</span><span class="oc-open">'+o[1].toFixed(2)+'</span><span class="oc-sep">/</span><span class="oc-open">'+o[2].toFixed(2)+'</span>';
  // 即时
  var curHtml = '<span class="oc-cur">'+cur[0].toFixed(2)+'</span><span class="oc-sep">/</span><span class="oc-cur">'+cur[1].toFixed(2)+'</span><span class="oc-sep">/</span><span class="oc-cur">'+cur[2].toFixed(2)+'</span>';
  // 分歧
  var divStr = '';
  for(var i=0;i<3;i++){
    var cls = d[i] < -0.3 ? 'oc-pct-down' : (d[i] > 0.3 ? 'oc-pct-up' : 'oc-pct-flat');
  ...
  return '<div class="odds-combined"><div class="oc-line"><span class="oc-label">初</span>'+openHtml+'</div>'+
    '<div class="oc-line"><span class="oc-label">即</span>'+curHtml+'</div>'+
    '<div class="oc-line oc-div"><span class="oc-label">分</span>'+divStr+'</div></div>';
}
...
function switchTab(tab){
  ...
  // 切换表
  document.getElementById('table-wrap-beidan').style.display = (tab === 'beidan' ? 'block' : 'none');
  document.getElementById('table-wrap-hkjc').style.display = (tab === 'hkjc' ? 'block' : 'none');
  applyFilters();
}
...
function applyFilters(){
  ...
  var filtered = allMatches.filter(function(m){
    ...
    if(activeTab==='beidan' && m.source!=='beidan') return false;
    if(activeTab==='hkjc' && m.source!=='hkjc') return false;
    return true;
  });
  ...
  if(activeTab==='beidan'){
    renderBeidanTable(filtered);
  } else {
    renderHkjcTable(filtered);
  }
}

function renderBeidanTable(matches){
...
function renderHkjcTable(matches){
  var tbody = document.getElementById('hkjcBody');
  tbody.innerHTML = '';
  matches.forEach(function(m){
    var tr = document.createElement('tr');
    tr.innerHTML =
      '<td>'+fmtTime(m.match_time)+'</td>'+
      '<td><span class="tag tag-hkjc">马会</span></td>'+
      '<td class="team-name">'+m.home_team+'</td>'+
      '<td class="score-cell">'+(m.score||'-')+'</td>'+
      '<td class="team-name">'+m.away_team+'</td>'+
      '<td class="odds-cell">'+renderHkjcOddsFull(m.hkjc_comparison)+'</td>';
    tbody.appendChild(tr);
  });
  document.getElementById('matchCount').textContent = matches.length+' 场';
}
...
fetch('data/results.json?v='+Date.now())
  ...
  .then(function(data){
    ...
    document.getElementById('table-wrap-beidan').style.display='block';
