...
// 加权排序: score = α * normalized_poisson + (1-α) * normalized_h2h_freq
function weightedSort(scores, home, away, alpha) {
  const key = [home, away].sort().join('||');
  const freq = (typeof H2H_SCORE_FREQ !== 'undefined' && H2H_SCORE_FREQ[key]) || {};
...
function openPoissonModal(record) {
  ...
  function render() {
    ...
    const sorted = weightedSort(scores, record.home, record.away, alpha);
...
function renderBarChart(top20, actualScore, over25, totalGoals) {
  ...
  // 按主胜/平局/客胜分组
  const groups = [
    { key: 'home', label: '主胜', cls: 'score-home', icon: '🏠', items: [] },
    { key: 'draw', label: '平局', cls: 'score-draw', icon: '🤝', items: [] },
    { key: 'away', label: '客胜', cls: 'score-away', icon: '🚩', items: [] }
  ];

  top20.forEach(s => {
    if (s.home > s.away) groups[0].items.push(s);
    else if (s.home === s.away) groups[1].items.push(s);
  ...
  groups.forEach(g => {
    ...
    // 计算该组泊松概率合计（用top20近似）
    const groupProb = g.items.reduce((sum, s) => sum + s.prob, 0);
    const groupPct = (groupProb * 100).toFixed(1);

    html += `<div class="bar-group-header ${g.cls}">
      <span class="group-icon">${g.icon}</span>
      <span class="group-label">${g.label}</span>
      <span class="group-subtotal">${groupPct}%</span>
    </div>`;

    g.items.forEach(s => {
      const barPct = Math.min(100, (s.prob * 100 * 6).toFixed(1)); // scale Poisson prob to visible bar
      const isHit = s.label === actualScore;
      const cls = isHit ? 'bar-hit' : '';
      const poissonPct = (s.prob * 100).toFixed(2);
      const mixedPct = (s.mixed * 100).toFixed(1);
      const freqTag = s.leagueCount > 0
        ? `<span class="bar-freq-tag" title="该联赛历史出现${s.leagueCount}次">📊${s.leagueCount}</span>`
        : '';

      html += `<div class="bar-row ${cls} ${g.cls}">
        <span class="bar-label">${s.label}</span>
        <div class="bar-track"><div class="bar-fill" style="width:${barPct}%"></div></div>
        <span class="bar-poisson">${poissonPct}%</span>
        <span class="bar-mixed">综合 ${mixedPct}%</span>
        ${freqTag}
...
function loadDate() {
  ...
  records.forEach((r, i) => {
    ...
    const kickoff = (!r.kickoff || r.kickoff === '待定') ? '-' : r.kickoff.substring(5, 16);
