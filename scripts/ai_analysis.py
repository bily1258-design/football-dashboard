...
<link rel="stylesheet" href="style.css?v=20260722vb1">
<script src="script.js?v=20260722vb1"></script>
</head>
<body>
<div class="container">
  <header>
  ...
  <div id="betTracker" style="display:none">
    ...
    <table id="betTable">
      <thead><tr><th>时间</th><th>比赛</th><th>方向</th><th>赔率</th><th>注额</th><th>结果</th><th>盈亏</th><th></th></tr></thead>
  ...
  <div id="betModal">
    <div class="modal-content">
      ...
      <h3>💰 确认投注</h3>
      <p><label>比赛:</label> <span id="bmMatch"></span></p>
      <p><label>方向:</label> <span id="bmOutcome"></span></p>
      <p><label>赔率:</label> <span id="bmOdds"></span></p>
      <p><label>Kelly建议:</label> <span id="bmKelly" style="color:#2563eb;font-weight:600"></span></p>
      <p><label>注额:</label> <input type="number" id="bmStake" step="0.1" min="0" style="width:80px;padding:4px 8px;border:1px solid #ccc;border-radius:4px"></p>
      <div class="modal-actions">
        <button onclick="confirmBet()" style="background:#2563eb;color:#fff;border:none;padding:8px 20px;border-radius:6px;cursor:pointer;font-weight:600">确认投注</button>
        <button onclick="closeBetModal()" style="background:#e5e7eb;color:#333;border:none;padding:8px 20px;border-radius:6px;cursor:pointer;margin-left:8px">取消</button>
      </div>
    </div>
  </div>
</div>
...
.vb-neg{color:#dc2626}
/* 投注弹窗 */
#betModal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:1000}
#betModal .modal-content{background:#fff;border-radius:12px;padding:24px;max-width:400px;margin:15% auto;box-shadow:0 8px 32px rgba(0,0,0,0.2);position:relative}
#betModal .modal-close{position:absolute;top:12px;right:16px;font-size:22px;cursor:pointer;color:#999}
#betModal h3{margin-bottom:16px;color:#1a2332}
#betModal p{margin:8px 0;font-size:14px}
#betModal label{display:inline-block;width:80px;color:#667788}
.modal-actions{margin-top:16px;text-align:right}
/* 投注追踪器 */
#betTracker{background:#fff;border-radius:8px;padding:16px;margin-bottom:16px;border:1px solid #d0d4dc}
#betTracker h3{font-size:16px;margin-bottom:12px;color:#1a2332}
#betSummary{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:12px;padding:10px;background:#f8f9fb;border-radius:6px}
.bs-row{display:flex;flex-direction:column;align-items:center;min-width:70px}
.bs-label{font-size:11px;color:#667788}
.bs-val{font-size:16px;font-weight:700;color:#1a2332}
.bs-profit{color:#16a34a}
.bs-loss{color:#dc2626}
.bs-empty{color:#99aabb;font-size:13px;padding:10px;text-align:center;width:100%}
#betTable{width:100%;border-collapse:collapse;font-size:12px;margin-top:8px}
#betTable th{background:#f0f2f5;padding:6px 8px;font-size:11px;color:#667788}
#betTable td{padding:6px 8px;border-bottom:1px solid #e5e7eb}
.bt-pending{color:#f59e0b;font-weight:600}
...
.lp-row:hover{opacity:0.85}'''
    ...
    if os.path.exists(script_path):
        ...
        fallback = os.path.join(current_dir, 'script.js')
        if os.path.exists(fallback):
            with open(fallback, 'r', encoding='utf-8') as sf:
                js = sf.read()
        else:
            js = '// script.js placeholder'
