...
if __name__ == "__main__":
    ...
    # 同步赛果缓存到 football-dashboard 仓库（供Termux/GA使用）
    try:
        import shutil
        dashboard_bsd = os.path.join(os.path.dirname(os.path.abspath(__file__)),
            'football-dashboard', 'data', 'raw', 'bsd')
        os.makedirs(dashboard_bsd, exist_ok=True)
        cache_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'cache')
        # 复制当天的赛果缓存
        src = os.path.join(cache_dir, f"500com_results_{target.replace('-','')}.json")
        if os.path.exists(src):
            dst = os.path.join(dashboard_bsd, f"{target.replace('-','')}.json")
            shutil.copy2(src, dst)
            print(f"  📂 赛果同步到 dashboard: {dst}")
    except Exception as e:
        print(f"  ⚠️ 赛果同步失败: {e}")
        ...
        if build_result.returncode == 0:
            ...
            if 'nothing to commit' in commit_result.stdout:
                ...
                if push_result.returncode == 0:
                    ...
                    print(f"⚠️ git push失败: {push_result.stderr[:100]}")
            ...
            if push_db_result.returncode == 0:
                ...
                print(f"  ⚠️ DB推送失败: {push_db_result.stderr[:80]}")
        else:
            print(f"⚠️ build失败: {build_result.stderr[:100]}")
    except Exception as e:
        print(f"⚠️ 看板推送失败: {e}")
